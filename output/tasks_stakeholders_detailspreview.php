...
require_once("include/tasks_stakeholders_variables.php");
...
$page_layouts["tasks_stakeholders_detailspreview"] = $layout;
...
{
	$where = "";
		$formattedValue = make_db_value("task_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$where .= $pageObject->getFieldSQLDecrypt("task_id") . " is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt("task_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	task_id - 
		    ...
		    $value = $viewContainer->showDBValue("task_id", $data, $keylink);
			$row["task_id_value"] = $value;
			$format = $pSet->getViewFormat("task_id");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("task_id")))
				$class = ' rnr-field-number';
			$row["task_id_class"] = $class;
	//	stakeholder_id - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("tasks_stakeholders", "detailspreview"));
