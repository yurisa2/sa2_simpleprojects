	...
	function buildControl($value, $mode, $fieldNum, $validate, $additionalCtrlParams, $data)
	{
		...
		switch( $dateEditType )
		{
			...
			case EDIT_DATE_SIMPLE_DP:
				...
				$ret= '<input id="'.$this->cfield.'" '.$this->inputStyle.$classString.' type="Text" name="'.$this->cfield.'" value="'.$ovalue.'">';								
			...
			case EDIT_DATE_DD:
				...
				$retday='<select '.$classString.' id="day'.$this->cfield.'" '.$dayStyle.$alt.'name="day'.$this->cfield.'" >'.$initDayOpt.'</select>';
				$retmonth='<select '.$classString.' id="month'.$this->cfield.'" '.$monthStyle.$alt.'name="month'.$this->cfield.'" >'.$initMonthOpt.'</option></select>';
				$retyear='<select '.$classString.' id="year'.$this->cfield.'" '.$yearStyle.$alt.'name="year'.$this->cfield.'" >'.$initYearOpt.'</select>';
				...
				if ( $this->pageObject->getLayoutVersion() == BOOTSTRAP_LAYOUT ) 
					$setHiddenElem = $classString . ' style="display:none; padding-left: 20px;"';
				...
				if( $this->pageObject->getLayoutVersion() == BOOTSTRAP_LAYOUT )
					$ret = '<div class="bs-date-control form-inline">' . $ret . '</div>';
			...
			default: //	case EDIT_DATE_SIMPLE:
				...
				echo '<input id="'.$this->cfield.'" type=text '.$classString.' name="'.$this->cfield.'" '.$this->inputStyle.' value="'.runner_htmlspecialchars($ovalue).'">';
		}
