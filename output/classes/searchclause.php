...
class SearchClause
{
	...
	public function buildItegratedWhere($fieldsArr, $editControls = null) 
	{
		...
		if( count($srchFields) )
		{
			...
			{	
				if( in_array($srchF['fName'], $fieldsArr) && is_null( $this->customFieldSQLConditions[ $srchF['fName'] ] ) )
				{
	...
	protected function getSimpleSearchWhere($fieldsArr, $editControls)
	{
		...
		if( $simpleSrch != null && strlen($simpleSrchField) && is_null( $this->customFieldSQLConditions[ $simpleSrchField  ] ) )
		{
		...
		foreach($simpleSrchArr as $ind => $simpleSrchItem)
		{
			...
			{
				if( !is_null( $this->customFieldSQLConditions[ $this->searchFieldsArr[$i] ] ) )
					continue;
