...
require_once("include/events_stakeholders_variables.php");
...
$page_layouts["events_stakeholders_detailspreview"] = $layout;
...
if($mastertable == "events")
{
	$where = "";
		$formattedValue = make_db_value("event_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$where .= $pageObject->getFieldSQLDecrypt("event_id") . " is null";
	else
		$where .= $pageObject->getFieldSQLDecrypt("event_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	event_id - 
			...
			$row["event_id_value"] = $value;
			$format = $pSet->getViewFormat("event_id");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("event_id")))
				$class = ' rnr-field-number';
			$row["event_id_class"] = $class;
	//	stakeholder_id - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("events_stakeholders", "detailspreview"));
