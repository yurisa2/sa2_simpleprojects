...
//	var $pageTypesInMenuForThisTable = array();
	...
	function __construct(&$menuItemInfo, &$menuNodes, &$menuParent, &$menuTableMap, $pageObject, $menuId )
	{	
	...
	{			
		...
		while( $menuNodesIndex < count( $menuNodes ) )
		{
			...
			// adds to parent
			++$menuNodesIndex;
			$this->AddChild( new MenuItem($menuNodes[$i], $menuNodes, $this, $this->menuTableMap, $this->pageObject, $this->menuId ) );
	...
	function getIconHTML() 
	{
		...
		{
			return '<span class="glyphicon '.$this->icon.'"></span>';
		} 
		else if ( $this->iconType == ICON_FILE )
		{
			return '<img src="images/menuicons/'.$this->icon.'">';
		}
	...
	{	
		...
		{
			if( $menuItemId == $this->id && $this->menuTableMap[ $this->table ][ strtolower($this->pageType) ] > 1 )
			{
	...
	{	
		...
		if( $this->isBootstrap() && $groupOnlyMode )
		{
