...
function checkTableName($shortTName, $type=false)
{
	...
	if ("projects" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("stakerholders" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("projects_stakeholders" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tasks" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tasks_stakeholders" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("events" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
{
	$arr = array();
	$strPerm = GetUserPermissions("projects");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="projects";
	}
	$strPerm = GetUserPermissions("stakerholders");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="stakerholders";
	}
	$strPerm = GetUserPermissions("projects_stakeholders");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="projects_stakeholders";
	}
	$strPerm = GetUserPermissions("tasks");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="tasks";
	}
	$strPerm = GetUserPermissions("tasks_stakeholders");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="tasks_stakeholders";
	}
...
function ReadUserPermissions($userID = "")
{
	...
	$sql = "select ". $rConnection->addFieldWrappers( "TableName" )
		...
		." from ". $rConnection->addTableWrappers( "ugrights" )
...
function guestHasPermissions()
{
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("projects",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("stakerholders",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("projects_stakeholders",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("tasks",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("tasks_stakeholders",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("events",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("admin_rights",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("users",$_SESSION["UserRights"]["Guest"]))
		return true;
...
function SetLangVars($xt, $prefix, $pageName = "", $extraparams = "")
{
	...
	$xt->assign("EnglishLANGLINK", $isBootstrap || "English" != $currentLang);
